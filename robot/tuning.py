...
def _f(path, lo, hi, live=True):
    return Param(path, "float", lo=lo, hi=hi, live=live)


def _i(path, lo, hi, live=True):
    return Param(path, "int", lo=lo, hi=hi, live=live)


def _b(path, live=True):
    return Param(path, "bool", live=live)


def _e(path, choices, live=True):
    return Param(path, "enum", choices=tuple(choices), live=live)


def _t(path, live=True):
    return Param(path, "text", live=live)
...
def _motor(side: str) -> Tuple[Param, ...]:
    ...
    return (
        _b(f"drive.{side}.inverted"),
        _f(f"drive.{side}.neutral_angle", -90, 90),
        _f(f"drive.{side}.max_angle", -90, 90),
        _f(f"drive.{side}.min_angle", -90, 90),
        _f(f"drive.{side}.deadband", 0, 0.5),
        _f(f"drive.{side}.max_forward", 0, 1),
        _f(f"drive.{side}.max_reverse", 0, 1),
        _i(f"drive.{side}.channel", 0, 15, live=False),
    )
...
def snapshot(cfg: RobotConfig) -> Dict[str, Any]:
    ...
    return {p.path: get(cfg, p.path) for p in PARAMS}
...
def needs_restart(paths) -> list:
    """Which of these applied paths won't take effect until the robot restarts."""
    return sorted(p for p in paths if p in BY_PATH and not BY_PATH[p].live)
