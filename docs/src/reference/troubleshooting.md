...
| The heading works on RVC but drifts over a run | The chip's yaw in this mode may not be magnetometer-referenced. Do the landmark check in [Wiring](wiring.md#the-imu-which-wire-it-is-on): if it has drifted tens of degrees after a few minutes of driving, it is dead-reckoning — set `heading_source=gps`, or go back to `imu.mode=i2c` where the calibration level tells you when the compass is trustworthy. |
