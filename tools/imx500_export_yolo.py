...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(f"\nNext, on the Raspberry Pi (needs `sudo apt install imx500-tools`):\n"
          ...
          f"Then calibrate range against THIS network — a pair measured on\n"
          f"another model, or on the edge_impulse backend, does not carry over:\n"
          f"    tools/detector_selftest.py --backend imx500 \\\n"
          f"        --imx500-model /path/to/network/network.rpk \\\n"
          f"        --imx500-labels /path/to/labels.txt \\\n"
          f"        --distance <tape measure, m>\n"
          f"    # -> prints RS_VISION_RANGE_AT_M / RS_VISION_RANGE_SIZE")
    return 0
