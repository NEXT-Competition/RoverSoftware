# How the detector in model/ was trained and exported: a Google Colab notebook,
# flattened to one file so it reviews and diffs like the rest of the tree.
#
# NOT IMPORTABLE, and not meant to be. It carries `!shell` and `%magic` lines,
# so `python train_assip_model.colab.py` is a SyntaxError — hence the
# `.colab.py` name, which keeps the syntax highlighting without claiming this
# is a module. Paste the cells into Colab (a GPU runtime and the team's Drive)
# rather than running it here; the `/content/gdrive/MyDrive/...` paths are
# somebody's Drive and are what you edit first.
#
# It is kept because model/ otherwise holds only OUTPUTS —
# ei-next-east-*.lite and imx500/network.rpk — with no record of the dataset,
# epochs or export settings behind them, which makes them impossible to
# reproduce or improve on. Read it as a lab notebook, not a pipeline: the
# sections below are successive attempts, including abandoned ones (two YOLO11n
# runs and a NanoDet detour), left in the order they were tried.
#
# --- 1. Export YOLO11n -> Grove Vision AI V2 ---
